<template>
  <div class="layout">
     <Layout>
       <mysql-menu :value="'4'" :tags="this.Tags"></mysql-menu>
...
export default {
  ...
  created () {
    this.get_mysql_alert_log(`tags=${this.$route.params.tags} `)
  ...
  methods: {
    get_mysql_alert_log (parameter) {
      getAlertLog(parameter).then(res => {
        this.data = res.data.results
        this.count = res.data.count
        console.log(this.data)
      }).catch(err => {
        this.$Message.error(`获取mysql日志信息错误 ${err}`)
      })
    },
    get_mysql_parameter (parameter) {
      console.log(parameter)
      this.get_mysql_alert_log(`tags=${this.$route.params.tags}&page=${parameter}`)
    },
    search () {
      if (this.logLevel === 'all') {
        this.get_mysql_alert_log(`tags=${this.$route.params.tags}&search=${this.logContent} `)
      } else { this.get_mysql_alert_log(`tags=${this.$route.params.tags}&log_level=${this.logLevel}&search=${this.logContent} `) }
    },
    clear_search () {
      this.logLevel = 'all'
      this.logContent = ''
